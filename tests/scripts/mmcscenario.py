...
def main():
    ...
    c_sc = ("Ge_sg227.ncmat;mos=20.0arcmin"
            ";dir1=@crys_hkl:5,1,1@lab:0,0,1;dir2=@crys_hkl:0,-1,1@lab:0,1,0")
    c_nobragg = ( 'solid::Al/1gcm3' )
    c_mp_notemp = ( 'phases<0.001*stdlib::'
                    'C_sg194_pyrolytic_graphite.ncmat;temp=100K'
                    '&0.999*solid::Al/1gcm3;temp=200K>' )
    c_mp_nobragg_notemp = ( 'phases<0.001*solid::Be/1gcm3;temp=100K'
                            '&0.999*solid::Al/1gcm3;temp=200K>' )
    ...
    testbad(c,'1.0 on 2.0mm',
            'Missing energy unit on "1.0". Possible units include Aa,'
            ' meV, eV, kT (=kB*temperature), BT (=Bragg threshold'
            ' in Aa, or 4Aa if not available).')
    testbad(c,'1.0ergs on 2.0mm',
            'Invalid energy unit on "1.0ergs". Possible units include Aa,'
            ' meV, eV, kT (=kB*temperature), BT (=Bragg threshold'
            ' in Aa, or 4Aa if not available).')
    ...
    test(c_mp_notemp,'1BT on 1mm')
    test(c_mp_nobragg_notemp,'1BT on 1mm')
    testbad(c_mp_notemp,'1kT on 1mm','Can not use kT as a unit for a'
            ' multi-phase material without a single well-defined temperature.')
    testbad(c_mp_nobragg_notemp,'1kT on 1mm','Can not use kT as a unit for a'
            ' multi-phase material without a single well-defined temperature.')
    test(c_mp_nobragg_notemp,'')
    test("void.ncmat",'1BT on 1m')
    test("void.ncmat",'1BT on 1mfp')
    testbad(c,'100 on 1m',
            'Missing energy unit on "100". Possible units include Aa,'
            ' meV, eV, kT (=kB*temperature), BT (=Bragg threshold'
            ' in Aa, or 4Aa if not available).')
    test(c,'1Aa on 1m')
