...
#FIXME: Can this be made into a meaningful test?? It might be better with a new
#cmdline utility (+jupyter compatible) which can be used to investigate the
#expansions? Or as a dev-only simplebuild util?
...
def do_test( cfgstr, atomlbl = None, do_plot = None):
    ...
    print("====================================")
    print()
    print("Reproduce with command: {}".format(build_cmdstr(cfgstr = cfgstr,
                                                           atomlbl = atomlbl)))
    query = ['vdos','expand',cfgstr]
    if atomlbl is not None:
        query.append(atomlbl)
    print(f"Query: ncrystal_query {shlex.join(query)}")
    data = ncquery(query,huge_arrays=True)
    assert set(data.keys()) == {'input','output'}
    data_in = data['input']
    data = data['output']
    print("alphaRange : ",data['alphaRange'])
    print("betaRange : ",data['betaRange'])
    kT = data['kT']

    #FIXME: Add some meaningful tests here.
    gns = {nm1 + 1: gn for nm1, gn in enumerate(data["Gn"])}

    gnmax = max(gns.keys())

    #Prepare Gn(beta) functions:
    nvals = [ nm1+1 for nm1 in range(gnmax) ]
    gnpts = []
    ...
    def plot_nselect(n):
        ...
        return not ( nfreq > 1 and n!=gnmax and n%nfreq!=0 )
    ...
    if do_plot:
        ...
        for pseudocurve, col, showpts in [('combinedGnFct',col_combinedGnFct,True),
            ...
            print(f"pseudocurve: {pseudocurve}: betavals=",pc_fct_b)
        ...
        for a in [ax,axdiff]:
            a.axvline(x=data['betaRange'][0], color="red", linestyle=":",
                      label="\u00b1betaRange")
            a.axvline(x=data['betaRange'][1], color="red", linestyle=":")
    ...
    if do_plot:
        ...
        for n in nvals:
            ...
            ax.plot(alphagrid,fnalpha(alphagrid,n),'x',color=col)
        ...
        for a in [ax,axdiff]:
            ...
            a.axvline(x=data['alphaRange'][1], color="red", linestyle="--",label="alphaMax")
...
def build_cmdstr(cfgstr,atomlbl=None):
    import os
    import sys
