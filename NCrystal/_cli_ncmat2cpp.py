...
#NB: Important that this script can work WITHOUT any imports except std modules,
#since the file is used as a script from the NCrystal CMake code to embed the
#data library.
...
def parseArgs( argv = None, *, return_parser = False ):
    import argparse
    import sys
    import os
    if argv is None:
    ...
    while argv and '--runasstandalonescript' in argv[1:]:
        run_as_standalone_script = True
        argv.remove('--runasstandalonescript')
...
#    if called_from_cli_script:#FIXME
#        def load_data( f ):
#            import pathlib
#            f = pathlib.Path(f)
#            if not f.exists():
#                #should already have been checked, but to be safe:
#                raise SystemExit('Error: File not found: %s'%f)
#            return dict( name = pathlib.Path(f),
#                         read_text_function = f.read_text,
#                         read_bytes_function = f.read_bytes )
#    else:
#        from .misc import AnyTextData
