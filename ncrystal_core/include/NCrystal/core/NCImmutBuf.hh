...
namespace NCRYSTAL_NAMESPACE {
  ...
  public:
    ...
    constexpr bool empty() const noexcept { return this->isEmpty(); }//fixme: unit-test
    ...
    //Destructor (does not work unless in remote mode):
    ~ImmutableBuffer();
...
namespace NCRYSTAL_NAMESPACE {
  ...
  inline void ImmutableBuffer<LBMS,BA,MD>::initBuffer( const char * src, std::size_t len )
  {
    ...
    if ( len > buffer_local_size ) {
      ...
      new(&m_data[0]) TRemotePtr(std::move(std::make_shared<const RemoteBuf>(src,len)));//fixme: std::move not needed?
