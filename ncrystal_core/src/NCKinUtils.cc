...
NC::PairDD NC::convertAlphaBetaToDeltaEMu(double alpha, double beta, double ekin, double kT )
{
#ifndef NDEBUG
  auto alim = getAlphaLimits(ekin/kT,beta);
  nc_assert(valueInInterval(alim.first,alim.second,alpha));
#endif
  nc_assert( ekin >= 0.0 );
  nc_assert( kT > 0.0 );
  nc_assert( alpha >= 0.0 );
  nc_assert( beta*kT >= -ekin );
  const double delta_e = beta * kT;
  const double ekinfinal = ekin + delta_e;
  const double denom = 2.0*std::sqrt(ekin * ekinfinal);
  if ( !denom )
  ...
  double mu =  ( ekin + ekinfinal - alpha*kT ) / denom;
