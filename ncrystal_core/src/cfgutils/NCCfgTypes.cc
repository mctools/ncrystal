...
NC::Optional<std::pair<double,NC::Cfg::ValDbl_ShortStrOrigRep>>
NC::Cfg::units_temperature::parse(StrView sv)
{
...
void NC::Cfg::units_length::listAvailableUnitsNoDefault(std::ostream& os )
{
  os << "Aa, nm, mu, mm, cm, m";
}
...
NC::Optional<std::pair<double,NC::Cfg::ValDbl_ShortStrOrigRep>>
NC::Cfg::units_length::parse(StrView sv)
{
  auto opt_p = unitSplit(sv);
  if (!opt_p.has_value())
    return NullOpt;
...
NC::Optional<std::pair<double,NC::Cfg::ValDbl_ShortStrOrigRep>>
NC::Cfg::units_purenumberonly::parse(StrView sv)
{
  auto opt_p = unitSplit(sv);
  if ( !opt_p.has_value())
    return NullOpt;
...
NC::Optional<std::pair<double,NC::Cfg::ValDbl_ShortStrOrigRep>>
NC::Cfg::units_angle::parse(StrView sv)
{
  auto opt_p = unitSplit(sv);
  if (!opt_p.has_value())
    return NullOpt;
...
{
  if ( !strrep.has_value() )
    NCRYSTAL_THROW2(BadInput,"Error - StrView without value provided"
                    " for parameter \""<<parname<<"\"");
  auto badchar_strrep = findForbiddenChar( strrep,
                                           forbidden_chars_value_strreps,
