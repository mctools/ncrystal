...
bool NC::MatCfg::has_extinction() const { return CfgManip::get_extn( m_impl->readVar(Cfg::VarId::extn) ).enabled(); }
NC::Cfg::ExtinctionCfgData NC::MatCfg::get_extinction() const { return CfgManip::get_extn( m_impl->readVar(Cfg::VarId::extn) ); }
...
void NC::MatCfg::set_extinction( const Cfg::ExtinctionCfgData& v )
{
  Cfg::ExtinctionCfg val( no_init );
  if ( v.enabled() ) {
    val = Cfg::ExtinctionCfg( Cfg::ExtinctionCfg::detail_from_varbuf_t(),
                              v.detail_accessRawData() );

  }
  m_impl.modify()->setVar( val, &CfgManip::set_extn );
}
