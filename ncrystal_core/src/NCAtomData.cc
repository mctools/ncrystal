...
NC::AtomData::AtomData( SigmaBound incXS, double cohSL, double captureXS, double avrMassAMU, unsigned Z, unsigned A )
  : m_m(avrMassAMU), m_ixs(incXS.val), m_csl(cohSL), m_axs(captureXS), m_classify(A), m_z(Z)
{
  nc_assert(m_z>0);
...
NC::AtomData::AtomData( const NC::AtomData::ComponentList& components )
  : m_m(0.0), m_ixs(0.0), m_csl(0.0), m_axs(0.0), m_classify(0), m_z(0)
{
  ...
  if (components.size()==1) {
    ...
    nc_assert_always(!!components.front().data);
    ...
    if (!!m_components) {
      ...
      m_components = std::make_unique<Component[]>(nc);
#else
      m_components = decltype(m_components)(new Component[nc]());
#endif
    ...
    if ( commonZ && ( commonA || allNaturalElements ) ) {
      ...
      for ( unsigned i = 1; i < nc; ++i ) {
        nc_assert_always(m_components[i].data);
        const AtomData& cc = *m_components[i].data;
      ...
      m_m = sum_wm.sum()*w_correction_factor;
...
void NC::AtomData::descriptionToStream(std::ostream& os, bool includeValues) const
{
  ...
  os<<"(cohSL="<<coherentScatLenFM()<<"fm"
    <<" cohXS="<<coherentXS().val<<"barn"
    <<" incXS="<<incoherentXS().val<<"barn"
    <<" absXS="<<captureXS()<<"barn"
    <<" mass="<<averageMassAMU()<<"amu";
  if (isElement())
    os<<" Z="<<Z();
  if (isSingleIsotope())
    os<<" A="<<A();
...
NC::SigmaFree NC::AtomData::boundToFreeXS( SigmaBound sb ) const
{
  double tmp = m_m / ( m_m + const_neutron_mass_amu );
  return SigmaFree{ tmp * tmp * sb.val };
}
