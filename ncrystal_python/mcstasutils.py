...
import importlib
try:
    NC = importlib.import_module('NCrystal', package=str(pathlib.Path(__file__).parent.resolve()))
except (ModuleNotFoundError,ImportError):
    #Fall back to standard search pattern:
...
def cfgstr_detect_components( cfgstr ):
    ...
    component names (e.g. "inelas", "coh_elas", "sans", etc.) and the associated
    ...
    probecfgstr = NC.normaliseCfg(cfgstr) + ';vdoslux=0'
    res=[]
    for ct in NC.standard_comp_types:
        extracfg = f';comp={ct}'
        if not NC.createScatter( probecfgstr + extracfg ).isNull():
            res.append( ( NC.normaliseCfg(cfgstr+extracfg), ct.replace('_','') ) )
    return res
...
def cfgstr_2_unioncfg( *, cfgstr, split_by_physics = False ):
    ...
    if split_by_physics:
        ...
        return inv_pen_depth_2200, [(NC.normaliseCfg(cfgstr),'total')]
...
def cfgstr_2_union_instrument_code( *, cfgstr, name, split_by_physics = False ):
    ...
    res = f"""
/*
   The following code was auto generated by NCrystal v{NC.__version__} via Python:
...
"""
    ...
    for proccfgstr, proctypename in physlist:
        ...
        if len(proccfgstr) > mcstas_string_max:
            warnings.warn(f'cfg_string might be too long for a McStas string: "{proccfgstr}"')
...
"""
    ...
    if len(procnamestr) > mcstas_string_max:
        warnings.warn(f'process_string might be too long for a McStas string: "{procnamestr}" '
...
/* End of auto generated code from NCrystal v{NC.__version__}. */
"""
...
def cfgstr_2_hkl(*, cfgstr, tgtformat, verbose=True, fp_format = '%.14g'):
    ...
    if data_name:
        for i,l in enumerate(NC.createTextData(data_name)):
    ...
    yield f'# File created by NCrystal v{NC.__version__}'
    yield '#'
        ...
        for ai in info.atominfos:
            if has_debye_temp:
                if ai.debyeTemperature is not None:
                    ...
                    _dt = NC.debyeTempFromIsotropicMSD( msd = ai.msd,
